...
// 三类“语义死循环”检测器：
//   generic_repeat        —— 同一工具、同样参数反复调
//   ping_pong             —— 两个操作 A→B→A→B 来回交替
//   global_circuit_breaker —— 同一调用持续无进展（结果一直不变）
...
// --- 配置（阈值为演示用调低，生产环境通常 Warning 10 / Critical 20 / 熔断 30）---

const HISTORY_SIZE = 30;       // 滑动窗口大小：只看最近 30 条行为
const WARNING_THRESHOLD = 5;   // 警告阈值
const CRITICAL_THRESHOLD = 8;  // 严重阈值
const BREAKER_THRESHOLD = 10;  // 全局熔断阈值（无进展）

// --- 指纹计算：确定性 JSON 序列化（key 排序）后哈希 ---
...
// 工具名 + 参数 → 指纹。参数顺序不同但内容相同，指纹一致。
export function hashToolCall(toolName: string, params: unknown): string {
  return hash(`${toolName}:${stableStringify(params)}`);
}
...
// 无进展连击：同名同参、且结果连续相同的次数（轮询 running running running…）
...
// --- 主检测函数（在 recordCall 之前调用：当前这一次调用还没进窗口）---
